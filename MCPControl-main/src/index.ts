...
class MCPControlServer {
  ...
  constructor(
    ...
    try {
      ...
      if (!config) {
        throw new Error('Invalid configuration: configuration could not be loaded');
      }

      const supportedProviders = ['keysender', 'autohotkey'];
      ...
      if (config.provider) {
        ...
        if (configuredProviders.length === 0) {
          throw new Error('Invalid configuration: no automation providers were configured');
        }

        const unsupportedProvider = configuredProviders.find(
          (provider) => !supportedProviders.includes(provider.toLowerCase()) && provider.toLowerCase() !== 'powershell' && provider.toLowerCase() !== 'clipboardy',
        );

        if (unsupportedProvider) {
          throw new Error(
            `Unsupported provider: ${unsupportedProvider}. Supported providers: ${supportedProviders.join(', ')}, powershell, clipboardy`,
          );
        }
      } else {
        throw new Error('Invalid configuration: no automation provider was configured');
      }
