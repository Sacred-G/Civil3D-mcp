...
let keysenderGetScreenSize: any = undefined;
try {
  const pkg = requirePkg('keysender');
  Hardware = pkg?.Hardware;
  keysenderGetScreenSize = pkg?.getScreenSize;
} catch {
  // Optional dependency not available -> will use fallbacks
}
...
export class KeysenderMouseAutomation implements MouseAutomation {
  ...
  private validatePositionAgainstScreen(position: MousePosition): MousePosition {
    ...
    const screenSize = typeof keysenderGetScreenSize === 'function' ? keysenderGetScreenSize() : { width: 8192, height: 4320 };
    if (
      position.x < 0 ||
  ...
  moveMouse(position: MousePosition): WindowsControlResponse {
    try {
      ...
      this.mouse.moveTo(position.x, position.y).catch((err: any) => {
  ...
  clickMouse(button: 'left' | 'right' | 'middle' = 'left'): WindowsControlResponse {
    try {
      ...
      // Start the asynchronous operation and handle errors properly
      this.mouse.click(mouseButton).catch((err: any) => {
      ...
      this.mouse
        ...
        .then(() => {
          ...
          setTimeout(() => {
            this.mouse
              .click()
              .catch((err: any) => this.logger.error('Error on second click of double-click', err));
          }, 50);
        })
        .catch((err: any) => this.logger.error('Error on first click of double-click', err));
  ...
  scrollMouse(amount: number): WindowsControlResponse {
    try {
      ...
      // Start the asynchronous operation and handle errors properly
      this.mouse.scrollWheel(amount).catch((err: any) => {
  ...
  dragMouse(
    ...
    try {
      ...
      this.mouse
        ...
        .then(() => {
          ...
          this.mouse
                ...
                this.mouse
                  ...
                  .then(() => {
                    ...
                    this.mouse
                      ...
                      .catch((err: any) => this.logger.error(`Error releasing ${button} button`, err));
                  })
                  .catch((err: any) => {
            ...
            .catch((err: any) => this.logger.error(`Error pressing ${button} button down`, err));
        })
        .catch((err: any) =>
      ...
      try {
        ...
        this.mouse
          .toggle(mouseButton, false)
          .catch((err: any) => this.logger.error(`Error releasing ${button} button during cleanup`, err));
      } catch (releaseError) {
    ...
    try {
      ...
      this.mouse
        ...
        .then(() => {
          ...
          this.mouse
            .click(mouseButton)
            .catch((err: any) => this.logger.error(`Error clicking ${button} button`, err));
        })
        .catch((err: any) => this.logger.error(`Error moving mouse to position ${x},${y}`, err));
