...
let getAllWindows: any = undefined;
try {
  const pkg = requirePkg('keysender');
  Hardware = pkg?.Hardware;
  keysenderGetScreenSize = pkg?.getScreenSize;
  getAllWindows = pkg?.getAllWindows;
} catch {
  // Optional dependency not available -> use fallbacks
}
...
export class KeysenderScreenAutomation implements ScreenAutomation {
  ...
  private findSuitableWindow(targetTitle?: string): {
    ...
    try {
      // Get all windows, fallback to an empty array when not available
      const allWindows = typeof getAllWindows === 'function' ? getAllWindows() : [];
      ...
      const windowsWithTitle = allWindows.filter(
        (w: any) => w.title && typeof w.title === 'string' && w.title.trim() !== '',
      );
